...
- **PWA(앱 설치)**: 주소창의 "앱 설치" 아이콘(데스크톱 크롬/엣지) 또는 "홈 화면에 추가"(모바일)로 앱처럼 설치 가능. 한 번 본 책은 오프라인에서도 열림 (`manifest.webmanifest` + `sw.js`). 새 책을 추가하면 `sw.js`의 `SHELL` 목록에 데이터 파일을 넣고 `CACHE_VERSION`을 올릴 것
